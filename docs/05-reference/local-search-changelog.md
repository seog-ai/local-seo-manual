...
Places bills a request at the **highest SKU touched by any field in its mask**, at the volume tier reached. A Text Search returning only `places.id` bills the Essentials (IDs Only) SKU, which is free; adding `rating` or `nationalPhoneNumber` to the same call moves it into the Enterprise tier — around $35 per 1,000 at the time of writing, from Google's published table. Adding `reviews` or attribute fields to a Place Details call moves it higher again.

Two cautions on the numbers. The dollar figures move, and the tier boundaries have moved with them, so **the mechanism is the durable fact here and the prices are not** — current per-SKU figures belong in [What Google's APIs actually cost](./what-googles-apis-cost.md) and should be read from Google's pricing page before you quote one to a client. And the date on this entry is the pricing table's own "Last updated" stamp rather than an announced change: the field-mask billing model long predates it, and what happened on this date was a restatement, not a new rule.
