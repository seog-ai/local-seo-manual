...
You do not win a logistics query by ranking. You win it by having the field filled in, correctly, and by it being the same everywhere ([the business entity](./the-business-entity.md)).
...
> The keyword you track **is** the question the assistant is asked ([how an AI assistant answers a local question](./how-ai-answers-a-local-question.md)).
...
The reusable version of this list lives in the appendix ([the local prompt corpus](../99-appendix/the-local-prompt-corpus.md)); the measurement method that consumes it is in [Part III](../03-advanced/ai-visibility.md).
