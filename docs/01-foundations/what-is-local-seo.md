...
| **AI local pack** | A generated local answer containing one or two businesses, in place of the pack | A subset of local queries, mostly on mobile | Undocumented |
...
> **Note** · There is a seventh thing on the page and it is not a ranking surface: ads. One tracked sample put local pack ads at 1% of queries in early 2025 and 22% by December 2025, with Local Services Ads going from 11% to 31%. You cannot rank your way into those slots — count them separately when you audit a page, or you will misread how much room is actually available.
...
**The AI local pack surfaces roughly a third as many businesses as the pack it replaces.** Sterling Sky's analysis of a fixed query set found AI local packs surfacing **5,943 unique businesses where the legacy 3-pack surfaced 18,330** — about 32% coverage. Two businesses in three simply vanish. In the same sample, AI local packs appeared on around 8% of tracked keywords, typically showed **two** businesses instead of three, and **had no call button**. Nothing is published on which businesses survive the cut or why, and Google's documentation does not acknowledge the surface at all.

**AI Overviews are common on local searches, and much rarer on local-intent ones.** Whitespark's 540-query sample found AI Overviews on about 68% of the set overall but on only about **15% of queries with clear local intent** — the transactional "I want a plumber now" phrasings. Local Falcon's May 2025 whitepaper (60,000 queries) put incidence at 40.2% with the same gradient: reasoning 59.9%, informational 58.3%, instructional 48.9%, transactional 39.8%, commercial 17.2%, navigational 10.5%. The two headline rates are not comparable — different query sets — but the gradient is. **The more the query looks like a purchase, the less likely a generated answer is to intercept it, for now.**

**The assistants recommend far fewer businesses than the pack does.** SOCi's 2026 index (~350,000 locations) reported recommendation rates of 1.2% for ChatGPT, 7.4% for Perplexity and 11% for Gemini, against 35.9% for the Google local 3-pack. The same study reported profile accuracy of roughly 68% on ChatGPT and Perplexity versus ~100% on Gemini — which tracks with Gemini being Maps-grounded — and only 45% overlap in retail between the brands that win Google local and the brands the assistants recommend. Treat the exact percentages as soft; the methodology is not fully published. Treat the *shape* as solid: these are different populations of winners.

**One surface has already been retired.** Google phased the public questions-and-answers panel off business profiles starting 3 December 2025, replacing it with Gemini-powered "Ask Maps". A capability that a whole category of tooling was built on stopped existing, and a surprising amount of current published advice still describes it as live. Dated facts are in [the local search changelog](../05-reference/local-search-changelog.md).
...
They diverge because they are grounded differently, not because any of them is broken. One measurement worth carrying: CiteLens found AI Mode and AI Overview citations — two surfaces from *the same company* — overlap only 13.7%. If Google cannot agree with itself about which sources to cite, an assistant from a different company disagreeing with your map pack is the normal condition, not an anomaly to be fixed.
...
- **Consistency of the entity's facts across the web** matters to every surface, and disproportionately to the AI ones. The 2026 Local Search Ranking Factors survey puts citation signals at roughly 7% of local-pack weight — small, and a poll of practitioners rather than a measurement. The case for doing the work anyway is now mostly an AI case, and it is a mechanism argument rather than a statistic: [Citations and NAP consistency](../02-core-practice/citations-and-nap.md) sets out what is and is not knowable there.
