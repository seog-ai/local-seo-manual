...
| --- | --- |
| Question text | The exact string, unbranded. A paraphrase is a different probe. |
| Engine | Findings never transfer between engines. |
| Anchor | The coordinates the question carried. |
| Timestamp | The index and the model both move under you. |
| Did the answer recommend businesses at all? | Refusals and "check Yelp" punts are a distinct outcome. |
| Named | Your business name appears in the answer text. |
| Cited | Your own domain appears in the source list. |
| Stance | *Recommended*, *listed*, *hedged*, or *negative*. |
| Every other business named, in order | The competitive set, chosen by the machine. |
| Every source cited | Where the answer came from. |
| The answer text itself | So a human can check the four judgements above. |
...
Named, cited and stance are three different wins. Two are measured over every answer and one is not, and getting that split wrong is how most AI-visibility numbers become meaningless.
...
**Citation rate** — your own domain among the cited sources, over *every* live answer. Strictly your domain, not a page about you. This is the "did the machine read my site" number, usually far lower than the mention rate. No dashboard I know of prints it beside the mention rate, this one included; you read it off the **Sources cited by AI** table, where your own domain is pinned at the top with the count of answers citing it.
...
Five runs buy direction and triage: zero-of-five and five-of-five are genuinely different states, and most probes sit at an extreme rather than the ambiguous middle. It is a working floor, and the app's rates use exactly that — a rolling window of the **five most recent live checks per keyword × engine**.

Two consequences. **Run the same number of checks in every cell**, or your rate is dominated by whichever cell you clicked most. And **the whole-matrix rate deserves less credit than its `n` suggests**: 120 observations implies a 4.5-point bar, but they are not independent — same business, same market, overlapping questions — and clustering inflates the true interval by an unknowable amount.
...
It is exquisitely sensitive to run count. With two runs and a true probability of 50%, the pair agrees half the time *by chance*, so consistency built from two-run cells is indistinguishable from coin-flipping; at five runs, coin-flipping produces all-agree only about 6% of the time. That sensitivity is why uniform `n` is a rule and not a preference.
...
**Then cells, by reading the answers.** Every rate here is a count of judgements about text made by string matching and a classifier, both fallible in ways you only see by reading.

One instrument-specific warning: in the app's matrix a cell shows the **latest** check's verdict while the tile above shows a **rate over the window**, so they will disagree — a business at 3/5 reads "Not mentioned" whenever the most recent run missed. Read the tile for the rate, the cell for the last observation, never one as the other.

## Calibrating the instrument

Every automated verdict is an approximation, and defensible measurement means knowing which way each errs. These are properties of the SEOG implementation, verified against the code on **2026-07-27**; your own tooling has equivalents, so go and find them.

**The name match is generous.** Substring containment, either direction, case-insensitive — so a business called *The Coffee Shop* matches an answer mentioning any coffee shop generically, and a cited page titled "Joe Coffee — Yelp" counts as a hit for *Joe Coffee*. Short and generic names inflate the mention rate. Your own-domain count on the sources table is the strict measure: read that, not the cell, when the question is "did the engine read *my site*".

**The "#N" beside a mentioned cell is a position in the source list, not a rank.** A business named in the prose but absent from the sources is inserted at the top of that list, so "#1" usually means "named in the answer". The genuine ordering figure is **Avg position** on the recommendations card — position among the businesses named. Two things called position on one screen; only one is about competitive standing.
...
**It is not causal.** You changed something and the rate moved; so did the index, possibly the model version, possibly the retrieval configuration — none announced, versioned or visible. The nearest available control is the same probe set run against a comparable business you did *not* work on, over the same window, both movements reported side by side. Not a trial. Far better than nothing.
...
**Engines change under you.** ChatGPT's opt-in device-location sharing shipped in late March 2026; measurements taken before it were of an engine that frequently had no reliable idea where the user was. *(Stated as of 2026-07; re-check before citing.)* Any series longer than a few months holds undocumented discontinuities — treat a step change as a suspect before a result.
...
2. Check them against your tracked keywords — anything active is already a probe row, because the AI check runs on active tracked keywords. Add what is missing with **Track**; the price is on the button.
...
4. Before running anything, write down what you expect the twin to do differently. A prediction you record is a test; one you make afterwards is a story.
...
**If it went wrong.** The twin will not save — same keyword, same location label and same language is the same row, so it needs a genuinely different **Search from** value. At your keyword quota: prune first, because a probe set you cannot afford to run five times is not a probe set.
...
**If it went wrong.** A cell shows **Sample**: that engine is not connected, its rows are fixture data excluded from every rate, and the column is unmeasured rather than zero. A pass returns fewer results than expected: checks can fail without stopping the batch, so re-run rather than scoring a miss as "not mentioned".
...
1. Read the **Presence** tile: the mention rate, and beneath it the "mentioned in X of Y recent live answers" that gives you the denominator. Write both down.
...
4. Write every cell as a fraction — `3/5`, not `60%` — with the worst-case error bar beside it. Take the two cells with the largest apparent gap and ask whether their intervals overlap; if they do, strike the comparison out before someone else does it for you.
...
7. Read **Who AI recommends alongside (or instead of) you**. Any recurring name you do not track is a competitor the machine chose for you — take it to [competitors](../02-core-practice/competitors.md).
8. Write the finding in three sentences carrying their denominators: *"Across N probes on three engines, five runs each, between DATE and DATE: mentioned in X of Y answers; own domain cited in Z. Absent on every engine for QUERY."*
...
**Reporting a percentage on five runs.** "60% AI visibility" reads as precision and is a 3/5 with a 65-point interval. Report fractions — they carry their own sample size. The same goes for averaging engines: 4/5 on one and 1/5 on another is two findings, not a mean.
...
**Treating a refusal as a loss.** An answer naming no businesses did not reject you, it left the question: it belongs in the mention-rate denominator and out of the recommendation denominator. Collapsing that is how "the AI never recommends us" gets said about an engine that recommends nobody for that query.

**Re-measuring immediately after a change and believing it.** The index has not caught up, the window still holds pre-change runs, and the interval is twenty points wide either way. Change, wait, then run a full uniform pass.
