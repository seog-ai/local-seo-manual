...
The useful question is **"who is above me, for which query, from where"** — and you have already paid for the answer. Every ranked search returns an ordered list, and the order *is* the measurement; reading who sits above your row costs nothing extra. Position is the cheap part of local data ([what Google's APIs cost](../05-reference/what-googles-apis-cost.md) explains why the rich profile fields are not).
