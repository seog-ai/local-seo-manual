...
**Doorway pages.** Somebody notices that "plumber in Croydon" is a query, generates forty city pages by find-and-replace, and ships them. Google's guidance on doorway pages — pages built to rank for many similar queries that all funnel to the same destination — has named this as spam for years, and it is among the more reliably punished patterns in local. The line is not whether the page carries a city name; it is whether the page describes something that exists: a real address, real staff, real service details, real photos, real hours. [Multi-location and franchise](../03-advanced/multi-location-and-franchise.md) does the structure at scale.
...
**There is no lab measurement for INP.** Of the three Core Web Vitals, Interaction to Next Paint can only come from real visitors — Chrome's field data. A lab test simulates a page load; it cannot simulate a human clicking things. So a low-traffic site legitimately has **no INP number at all**, and the honest display is a dash. A tool showing an INP figure for a site with almost no traffic invented it. Verified against Google's PageSpeed Insights behaviour, 2026-07.
...
**The query list will never sum to the total.** Google omits rare queries — ones made by very few people — to protect privacy. Your top-25 list is real, and it is a subset. "We rank for 25 queries" is wrong in a way that is hard to walk back.

**Choose the domain property.** A property registered for the whole domain covers every protocol and subdomain at once; a URL-prefix property covers exactly one variant, so `https://www.example.com/` misses `https://example.com/`. A large share of "Search Console shows no data" is a site measured through the wrong property — and an unverified property cannot be queried at all.
...
Search Console's interface shows *fresh* data, including the last two or three days. The API, asked the same question over the same dates, returns **final** data by default and silently drops those freshest days. Same property, same range, smaller numbers, no error, no footnote. Every tool built on the API that never changed the default understates the client's own dashboard, permanently, by a couple of days' worth. The fix is one switch — ask for all data rather than final; the parameter is named in [what Google's reporting hides](../05-reference/what-googles-reporting-hides.md). The Search performance card below already asks for all data, which is why its totals should line up.
