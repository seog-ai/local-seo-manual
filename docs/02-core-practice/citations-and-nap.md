...
For the map pack, less than the industry sells. The largest annual practitioner survey — [Whitespark's Local Search Ranking Factors](https://whitespark.ca/local-search-ranking-factors/), 2026 edition — puts **citation signals at roughly 7% of local-pack weight**, behind profile signals (~32%), reviews (~20%), on-page signals (~19%) and links (~15%). Two caveats matter more than the number. It is an **opinion survey** — expert consensus about a system none of the respondents can see inside — so treat it as the best available prior, not a finding. And **a small average weight is not a small effect**: citation influence looks threshold-shaped *(inference)*, invisible for most businesses and then expensive the moment there is a real contradiction.
