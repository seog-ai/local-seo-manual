...
The conservative practice stands either way — do not re-measure visibility in the days after a name, category or address edit, because you cannot separate an edit effect from a verification effect ([The profile is the product](./the-profile-is-the-product.md), [Suspensions and reinstatement](../03-advanced/suspensions-and-reinstatement.md)).

For a review reply, tier 1 means a read-back containing your own text — a success response on the write is not publication, which [Reviews](./reviews.md) makes a standing rule.
...
A working rule, marked as what it is: *(inference)* profile and review changes tend to show over weeks rather than days, so re-scan fortnightly at most and judge on three readings, not two — see [Building a tracked set that tells the truth](./choosing-what-to-track.md).
