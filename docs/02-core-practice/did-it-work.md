...
One trap sits inside the app: the **Top-3 coverage** trend line above the map plots your last scans of that keyword *at whatever detail level each was run*. Scan Quick in January and Detailed in February and the line shows a step that is an artefact of the preset. Pick one preset per keyword and stay on it; changing it starts a new baseline, and you say so.
...
**Tier 1 — did the change land?** Binary, fast, and the only one you fully control. A profile edit is submitted, reviewed by Google, then published — usually within about ten minutes for ordinary fields. Until you re-fetch you do not know which: the app shows a *"N profile edits applied since your last refresh — Google reviews edits before they go live"* notice, and the refresh settles it. If Google rejected the edit, the re-pull restores the old value and the fix reappears in your list. That is not a bug; that is the answer.

Name, category and address are a different class of field — they can put a listing into re-verification and out of Search and Maps for hours to days, so do not re-measure visibility inside that window ([The profile is the product](./the-profile-is-the-product.md), [Suspensions and reinstatement](../03-advanced/suspensions-and-reinstatement.md)). For a review reply, tier 1 means a read-back containing your own text — a success response on the write is not publication, which [Reviews](./reviews.md) makes a standing rule.
...
**There is no "profile views" number.** Google reports impressions in four separate counts — desktop Maps, desktop Search, mobile Maps, mobile Search — and "profile views" is their sum. A report built from one of them is a quarter of the truth, and such reports exist. The app sums the four into one **Profile views** series; if you build this yourself, sum them or say which you used. The mechanism is [What Google's reporting hides](../05-reference/what-googles-reporting-hides.md).

**Days with no data are omitted, not zeroed.** Google's daily series leaves out days it has nothing for, so anything reconstructing a calendar back-fills them as zero. Correct for arithmetic, and a trap at the recent end of the chart, because the window runs to today and Google reports with a lag. If the last day or two of every metric read zero while the preceding weeks look healthy, that is lag, not a collapse. *(Inference: the lag length is not documented — verify it on your own chart a few days later, watching those zeros fill in.)* That tail also drags each tile's trend badge, which compares the first week of the window against the last. Read the sparkline, not just the percentage.

**A click-to-call is not a call.** **Calls** counts taps on the call button. Whether the call connected, was answered, or lasted two seconds is not in the data — Google does not report it, so no tool has it. Likewise **Direction requests** counts people who asked for directions, not people who arrived, and **Website clicks** counts clicks out, which will not match your analytics *(inference on that gap's direction: consent banners and blockers make analytics the lower number)*. In the search-terms table below, low-volume terms come back with their count withheld and render as `<N`, so a total built from them is an upper bound — label it as one ([Building a tracked set that tells the truth](./choosing-what-to-track.md)).
...
3. Set the period to **28 days** and note each tile's total: Profile views, Calls, Website clicks, Direction requests, and Messages or Bookings where the category has them.
4. Look at the right-hand end of any sparkline. If the last day or two sit at zero while the rest of the window is healthy, that is lag rendered as zeros. Keep those days out of comparisons.
5. Switch to **90 days**, then **12 months**. Each tile's total *and* its trend badge move — the badge compares the window's first week to its last, so its meaning changes with the period.
6. For each tile, write what it does **not** tell you: calls = taps on the call button, not conversations; direction requests = intent, not arrivals; website clicks ≠ sessions in your analytics. Then find any `<N` in the search-terms table — a withheld count, real number below it.
