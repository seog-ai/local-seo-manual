...
- **There is a rate cap on profile edits**, and it is shared across every kind of edit rather than being per-field — [LSM-GBP-02 in *Write limits and failure modes*](../05-reference/write-limits-and-failure-modes.md) carries the measured number and the probe. It is generous enough that a human clicking cannot normally reach it, and easy to reach if you queue thirty changes and paste them all at once. Spread bulk work out.
...
Upload through the dashboard. Everything the manual says about photos holds identically: uploads are reviewed before they appear ([LSM-GBP-12](../05-reference/write-limits-and-failure-modes.md)), and you can delete photos you uploaded but not photos customers uploaded, cannot replace one atomically, and cannot reorder them ([LSM-GBP-10](../05-reference/write-limits-and-failure-modes.md)).
